// api/rejeitar.js — Rejeita item de curadoria via link do e-mail
// GET /api/rejeitar?id=UUID&token=bicalho2025
...
export default async function handler(req, res) {
  ...
  if (token !== TOKEN_VALIDO) {
    return res.status(403).send(paginaHtml('❌ Acesso negado', 'Token inválido.', false));
  }
  if (!id) {
    return res.status(400).send(paginaHtml('❌ Erro', 'ID não informado.', false));
  }
  ...
  try {
    ...
    return res.status(200).send(paginaHtml(
      '✗ Item rejeitado',
      'O item foi rejeitado e não aparecerá no portal do cliente.',
      false
    ));
  } catch(e) {
    return res.status(500).send(paginaHtml('❌ Erro', e.message, false));
  }
}

function paginaHtml(titulo, msg, ok) {
  return `<!DOCTYPE html>
...
<body style="font-family:'Helvetica Neue',Arial,sans-serif;background:#f5f5f0;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0">
  <div style="background:#fff;border-radius:16px;padding:40px 48px;text-align:center;max-width:480px;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
    ...
    <a href="https://curitibaem-movimento.vercel.app/admin.html"
       style="display:inline-block;background:#555;color:#fff;text-decoration:none;padding:10px 24px;border-radius:8px;font-size:14px;font-weight:700">
